...
const StyleCard = ({ data }: { data: any }) => {
  ...
  return (
    <div
      ...
      className={`group relative w-full h-full flex flex-col p-2 rounded-[20px] cursor-pointer transition-all duration-300 border ${isCurrent ? 'bg-zinc-900 border-purple-500/30' : 'bg-transparent border-transparent hover:bg-zinc-900/10'}`}
    >
      ...
      <div className="text-center px-1 flex-grow flex flex-col justify-start">
        <h3 className={`text-sm mb-1.5 tracking-wide ${isCurrent ? 'text-purple-400 font-bold' : 'text-white'}`}>{data.title}</h3>
    ...
    <div className="bg-[#0e0e0f] h-full text-white font-sans selection:bg-purple-500/30 overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-500 fade-in">
      ...
      <div className="flex-1 min-w-0 mr-2 ml-3 rounded-[30px] bg-black overflow-hidden relative shadow-2xl mt-2 mb-2 border border-zinc-900/50">
        ...
        <div className="h-full overflow-y-auto scrollbar-hide pb-32 scroll-smooth">
          ...
          {/* Nav */}
          <div className="flex justify-between items-center relative z-10 px-4 mt-4">
          ...
          />
            ...
            <div className="mb-8 flex gap-2">
              ...
              <button className="bg-transparent text-zinc-600 border border-zinc-800 px-8 py-2.5 rounded-full text-[10px] font-extrabold tracking-widest uppercase hover:text-white hover:border-zinc-600 transition-colors">Signature Sound</button>
            </div>
